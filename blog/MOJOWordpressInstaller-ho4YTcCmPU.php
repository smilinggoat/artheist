...
$MOJOConfiguration = '{
    "userName": "artheist.rec@gmail.com",
    "userPassword": "$P$BkCMgND6tWEeYhB.ELDdizqswzzG6z\/",
    "userEmail": "smilinggoat@gmail.com",
    "siteUrl": "http:\/\/www.artheist.co\/blog\/",
    "blogTitle": "Art Heist",
    "process": "install",
    "theme": false,
    "authTokenField": "fHnSRxl8ObzvgUhU",
    "authToken": "2b154e18f7a1dc5b7142441aff7078e0",
    "authSalt": "cE7whIYoGNf0Z8tJFKzL2hucYTit5hy2RqZOCqf6tAw4hFDqMj6JjT969f20oVcC",
    "hostId": "b7119342-0c17-11e2-8b3b-0030483370c8",
    "hostName": "BlueHost"
}';
...
class MOJOTestComplete extends Exception {

}

class MOJODeathException extends Exception {
 ...
 * @return [type]
 *
 * @throws Exception if $function is not a callable method.
 */
...
/**
 * testContext to see if we are in testing or not
 *
 * @return [bool] [if we are testing or not]
 */
	public function testContext() {
		if (defined('MOJO_TEST')) {
			return true;
		}
		return false;
	}
 ...
 */
	public static function formatSuccess() {
		return array(
			...
			'logs' => self::$logs,
			'softFailures' => self::$softFailures,
			// Looks like with a hard memory limit that Json Encode will fail out if we try to add the ini
			//'ini' => ini_get_all(),
			// This can be added back in, but since no one sees this stuff, why
 ...
 * @return [type]
 * @throws  MOJOTestComplete When testing, we need to pass the output up, so an exception is raised
 */
	public static function finish($output) {
		self::$cleanShutdown = true;

		// If testing, return value
		if (parent::testContext()) {
			throw new MOJOTestComplete(MOJOMarshaller::marshall($output));
		}
 ...
 * @return [type]
 *
 * @throws MOJOInstallFailedException if configuration options are missing.
 */
 ...
 *
 * @return [type]
 * @throws  MOJODeathException When testing, we need to pass the output up, so an exception is raised
 */
	protected function _authFailed() {
		...
		if ($this->testContext()) {
			...
			throw new MOJODeathException($output);
		}
 ...
 * @return [type]
 *
 * @throws MOJOContinueException if there isn't enough time to finish steps.
 */
 ...
 */
	protected function _startUp() {
		...
		$this->_remaining = array_diff($this->_steps, $completedStateKeys);
	}
 ...
 * @return [type]
 *
 * @throws MOJOInstallFailedException if core WordPress files are missing.
 */
 ...
 * @return [type]
 *
 * @throws MOJOInstallFailedException if server environment is incompatible.
 */
 ...
 */
	public function setUpSiteOptions() {
		...
		$this->mockable_add_option('frm_aff', 'mojo');
		if (isset($this->_configuration['hostId'])) {
			$this->mockable_add_option('mm_host', $this->_configuration['hostId']);
		}

		if (isset($this->_configuration['hostName'])) {
			$this->mockable_add_option('mm_brand', $this->_configuration['hostName']);
		}
	}
 ...
 */
	public function createAdminUser() {
		...
		$userId = username_exists($this->_configuration['userName']);

		if (empty($userId)) {
			$userPassword = wp_generate_password(256, false);
			$userId = wp_create_user($this->_configuration['userName'], $userPassword, $this->_configuration['userEmail']);
		}

		$user = $this->instantiate('WP_User', array($userId));
		...
		$wpdb->update($wpdb->users, $passwordUpdate, array('ID' => $userId));

		MOJOOutput::log("created admin user " . json_encode(array('userId' => $userId, 'update' => $passwordUpdate, 'userName' => $this->_configuration['userName'])));
...
class MOJOWordpressPluginHandler extends MOJOBaseHandler {
	...
	protected $_excludedPlugins = array('woocommerce/woocommerce.php');
 ...
 */
	public function activateAll() {
		...
		foreach ($plugins as $plugin => $pluginData) {
			...
			if (in_array($plugin, $this->_excludedPlugins)) {
				continue;
			}
